import { Calendar, Map, MapPin, PenBox, Verified } from 'lucide-react'
  ...
  return (
    <div className='relative py-4 px-6 md:px-8 bg-white'>
        <div className='flex flex-col md:flex-row items-start gap-5'> 
            ...
            <div className='w-32 h-32 border-4 border-white shadow-lg absolute -top-14 rounded-full'>
                <img src={user.profile_picture} alt='' className='absolute rounded-full z-2' />
            </div>

            <div className='w-full pt-16 md:pt-0.5 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    <div>
                        <div className='flex items-center gap-4'>
                            <h1 className='text-2xl font-bold text-gray-900'>{user.full_name}</h1>
                            <Verified className='w-5 h-5 -ml-1 mt-0.75 text-blue-600' />
                        </div>
                        <p className='text-gray-400 text-sm'>{user.username ? `@${user.username}` : 'Add a username'}</p>
                    </div>

                    {/* if the user is not on others profile that means he is opening his profile so we 
                    will give them the edit button */}
                    {!profileId && 
                        <button onClick={()=> setShowEdit(true)} className='flex items-center gap-2 border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-3xl font-medium transition-colors mt-4 md:mt-0 cursor-pointer'>
