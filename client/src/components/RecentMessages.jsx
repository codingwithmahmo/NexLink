...
const RecentMessages = () => {
    //add state for retrieving messages here
        ...
        <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
            {
                messages.map((message, index)=>(
                    <Link to={`/messages/${message.from_user_id.__id}`} key={index} className='flex items-start gap-2 py-2 hover:bg-slate-100 rounded-xl p-2'>
